...
con.connect(("172.21.145.31", 51244))
...
while not finish:
    ...
    if msgtype == 1:
        ...
        with open('EEG_Data.csv','wb') as f:
        ...
        if len(data1s) > channelCount * 1000000 / samplingInterval:
            ...
            for i in range(len(data1s)):
                data1s[i] =  data1s[i]*resolutions[i % channelCount]
            a=numpy.array(data1s)
            print len(data1s)
            print a.shape
            b=numpy.reshape(a,(500,63))
            print b.shape    
            with open("EEG_Data.csv",'ab') as f :
                cv=csv.writer(f)
                for i in b:cv.writerow(i)
